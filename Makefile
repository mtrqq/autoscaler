...
build-in-docker-arch-%: clean-arch-% docker-builder
	docker run ${RM_FLAG} -v `pwd`:/gopath/src/k8s.io/cluster-autoscaler/pkg/:Z autoscaling-builder:latest \
		bash -c 'cd /gopath/src/k8s.io/cluster-autoscaler/pkg && BUILD_TAGS=${BUILD_TAGS} LDFLAGS="${LDFLAGS}" make build-arch-$*'
