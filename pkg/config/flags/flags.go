...
func (p *AutoscalingFlags) AddFlags(fs *pflag.FlagSet) {
	...
	fs.Float32Var(&p.o.KubeClientOpts.KubeClientQPS, "kube-client-qps", float32(rest.DefaultQPS), "QPS value for kubernetes client.")
	...
	fs.DurationVar(&p.o.ScaleDownDelayAfterDelete, "scale-down-delay-after-delete", 0, "How long after node deletion that scale down evaluation resumes")
	...
	fs.DurationVar(&p.o.NodeDeletionBatcherInterval, "node-deletion-batcher-interval", 0*time.Second, "How long CA ScaleDown gather nodes to delete them in batch.")
	...
	fs.DurationVar(&p.o.NewPodScaleUpDelay, "new-pod-scale-up-delay", 0*time.Second, "Pods less than this old will not be considered for scale-up. Can be increased for individual pods through annotation 'cluster-autoscaler.kubernetes.io/pod-scale-up-delay'.")
	...
	fs.BoolVar(&p.o.ProactiveScaleupEnabled, "enable-proactive-scale-down", false, "Whether to enable/disable proactive scale-downs, defaults to false")
	...
	fs.IntVar(&p.o.ClusterSnapshotParallelism, "cluster-snapshot-parallelism", 16, "Maximum parallelism of cluster snapshot creation.")
	...
	fs.DurationVar(&p.o.NodeDeletionCandidateTTL, "node-deletion-candidate-ttl", time.Duration(0), "Maximum time a node can be marked as removable before the marking becomes stale. This sets the TTL of Cluster-Autoscaler's state if the Cluste-Autoscaler deployment becomes inactive")
	...
	fs.BoolVar(&p.o.DynamicResourceAllocationEnabled, "enable-dynamic-resource-allocation", true, "[Deprecated] Handle DRA (Dynamic Resource Allocation) objects, locked to true.")
  ...
  // Properties specific to the cloud provider, but kept for the sake of POC
	...
	fs.IntVar(&p.maxGracefulTerminationSec, "max-graceful-termination-sec", -1, "Maximum number of seconds CA waits for pod termination when trying to scale down a node. This flag is mutually exclusion with drain-priority-config flag which allows more configuration options.")
...
func (p *AutoscalingFlags) AutoscalingOptions() (config.AutoscalingOptions, error) {
	...
	if p.drainPriorityConfig != "" && p.maxGracefulTerminationSec == -1 {
	...
	if p.maxGracefulTerminationSec == -1 {
		p.maxGracefulTerminationSec = 10*60
	}
	...
	if p.o.ScaleDownEnabled == false {
		return config.AutoscalingOptions{}, fmt.Errorf("--scale-down-enabled flag is deprecated and will be removed in a future release")
	}
